...
APIKeyFile = dotenv_values(".env")
# ======改變.env讀法==========================
AccessToken = APIKeyFile["AccessToken"]
ChannelSecret = APIKeyFile["ChannelSecret"]
configuration = Configuration(access_token = AccessToken)
LineHandler = WebhookHandler(ChannelSecret)
# AzureAPIVersion = APIKeyFile["AzureAPIVersion"]
...
def azure_translate(user_input, to_language):
    if to_language == None:
        ...
        apikey = APIKeyFile["API_KEY"]
        endpoint = APIKeyFile["ENDPOINT"]
        region = APIKeyFile["REGION"]
